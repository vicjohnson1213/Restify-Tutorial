...
// Creates a new Friend in the database and responds with an array of all Friends in the database.
...
server.put('/friends/:id', function(req, res, next) {
    Friend.update({ _id: req.params.id }, req.params, function(err, friend) {
        ...
        // Find the Friend with the specified id and return the updated user to the caller.
        Friend.findById(req.params.id, function(err, friend) {
            if (err)
                res.send(err);
